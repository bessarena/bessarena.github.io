...
function filtrarFilmes(filmes) {
  ...
  return filmes.filter(filme => filme.overview && new Date(filme.release_date) > today && filme.poster_path !== null);
}

function renderizarFilmes(filmes, nome) {
  if (filmes.length === 0) {
    subscricoesContainer.innerHTML += `<h2>Nenhum filme encontrado para ${nome}.</h2>`;
    return;
  }

  subscricoesContainer.innerHTML += `<h2>Próximos filmes de ${nome}:</h2>`;
...
async function mostrarFilmes() {
  try {
    ...
    const filmesLimitados = filmesValidos.slice(0, 4);
    renderizarFilmes(filmesLimitados, 'Atores e Realizadores');
